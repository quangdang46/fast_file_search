...
fn setup_once() -> Result<(SharedFilePicker, SharedFrecency), String> {
    ...
    let canonical_path = ffs::path_utils::canonicalize(&big_repo_path)
...
fn bench_indexing(c: &mut Criterion) {
    ...
    let canonical_path = match ffs::path_utils::canonicalize(&big_repo_path) {
        Ok(p) => p,
        Err(e) => {
...
fn bench_grep_search(c: &mut Criterion) {
    ...
    let grep_parser = ffs::QueryParser::new(ffs::GrepConfig);
