...
pub fn restart_index_in_path(_: &Lua, new_path: String) -> LuaResult<()> {
    ...
    let canonical_path = ffs::path_utils::canonicalize(&path).map_err(|e| {
...
pub fn fuzzy_search_files(
    ...
    if results.items.is_empty() && query.contains(std::path::MAIN_SEPARATOR) {
        let pure_query = match &parsed.fuzzy_query {
            ffs_query_parser::FuzzyQuery::Text(t) => t.trim(),
            _ => query.trim(),
        };
...
pub fn live_grep(
    ...
    let parsed = ffs::grep::parse_grep_query(&query);
    let mode = match grep_mode.as_deref() {
        Some("regex") => ffs::GrepMode::Regex,
        Some("fuzzy") => ffs::GrepMode::Fuzzy,
        _ => ffs::GrepMode::PlainText, // "plain" or nil or unknown
    };
...
pub fn track_query_completion(_: &Lua, (query, file_path): (String, String)) -> LuaResult<bool> {
    ...
    std::thread::spawn(move || {
        ...
        let file_path = match ffs::path_utils::canonicalize(&file_path) {
            Ok(path) => path,
            Err(e) => {
