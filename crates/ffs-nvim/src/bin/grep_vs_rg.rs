...
/// - ffs: pre-indexed files + cached mmaps = near-zero overhead per search
...
use ffs::grep::{GrepSearchOptions, parse_grep_query};
use std::path::Path;
...
fn main() {
    ...
    eprintln!("  ffs: paginated search (50 matches) from warm mmap cache");
    ...
    eprintln!("  ffs pays this cost once at startup, then searches from warm cached mmaps.");
    eprintln!();
}
