...
pub const MCP_INSTRUCTIONS: &str = concat!(
    ...
    "\n",
    "### 2. Prefer plain text; use ffs_multi_grep for OR over literals\n",
    "Plain text search is faster and more reliable. Regex patterns like `.*`, `\\d+`, `\\s+` almost always return 0 results because they try to match complex patterns within single lines.\n",
    "Regex alternation with `|` IS supported (e.g. 'TODO|FIXME'), but for 2+ literal identifiers prefer ONE ffs_multi_grep call with literal patterns — same results, faster.\n",
    "\n",
    ...
    "## Output Format\n",
    "\n",
    "ffs_grep defaults to output_mode 'compact' (rg-style path:line:text rows, token-efficient).\n",
    "Pass output_mode 'content' when you need definition body context (struct fields, function signatures) —\n",
    "lines marked with | are definition body context, [def] marks definition files, and Read suggestions point to the most relevant file.\n",
