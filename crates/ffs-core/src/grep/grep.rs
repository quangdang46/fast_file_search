...
impl Matcher for RegexMatcher<'_> {
    ...
    #[inline]
    fn line_terminator(&self) -> Option<ffs_grep::LineTerminator> {
        if self.is_multiline {
            None
        } else {
            Some(ffs_grep::LineTerminator::byte(b'\n'))
        }
    }
}
...
impl Sink for PlainTextSink<'_> {
    ...
    fn finish(&mut self, _: &Searcher, _: &ffs_grep::SinkFinish) -> Result<(), Self::Error> {
        Ok(())
    }
}
    ...
    for chunk in files_to_search.chunks(chunk_size) {
        ...
        let chunk_results: Vec<(usize, &'a FileItem, Vec<GrepMatch>)> = chunk
            ...
            .map_init(
                ...
                // and an mmap slot for cache-miss large files.
...
pub(super) fn collect_grep_results<'a>(
...
pub(super) fn prepare_files_to_search<'a>(
    files: &'a [FileItem],
    constraints: &[ffs_query_parser::Constraint<'_>],
    options: &GrepSearchOptions,
    arena: crate::simd_path::ArenaPtr,
...
pub(crate) fn grep_search<'a>(
    ...
    let grep_text = if !matches!(query.fuzzy_query, ffs_query_parser::FuzzyQuery::Empty) {
        query.grep_text()
    } else {
