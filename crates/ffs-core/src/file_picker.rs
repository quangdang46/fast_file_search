...
impl FilePicker {
    ...
    pub fn fuzzy_search<'q>(
        ...
        let effective_query = match &query.fuzzy_query {
            ffs_query_parser::FuzzyQuery::Text(t) => *t,
            ffs_query_parser::FuzzyQuery::Parts(parts) if !parts.is_empty() => parts[0],
...
/// Enabled via the `mimalloc-collect` feature (set by ffs-nvim).
