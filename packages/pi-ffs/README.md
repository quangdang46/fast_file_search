...
A [pi](https://github.com/badlogic/pi-mono) extension that replaces the built-in `find` and `grep` tools with [FFF](https://github.com/dmtrKovalenko/ffs.nvim) — a Rust-native, SIMD-accelerated file finder with built-in memory.
...
```json
{
  "extensions": ["/path/to/ffs.nvim/packages/pi-fff/src/index.ts"]
}
```
...
```bash
pi -e /path/to/ffs.nvim/packages/pi-fff/src/index.ts
```
