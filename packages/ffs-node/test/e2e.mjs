...
describe("ffs-node", { concurrency: 1 }, () => {
  before(async () => {
  ...
  describe("grep", { concurrency: 1 }, () => {
    ...
    it("decodes before/after context lines", () => {
      ...
      const match = r.value.items.find(
        (m) => normalizePath(m.relativePath) === "packages/ffs-node/src/ffi.ts",
      );
      assert.ok(
        match,
        `expected a match in packages/ffs-node/src/ffi.ts, got: ${r.value.items
