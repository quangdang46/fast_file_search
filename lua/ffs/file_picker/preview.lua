...
local image = require('ffs.file_picker.image')
...
function M.setup(config)
  ...
  if not M.state.location_namespace then
    M.state.location_namespace = vim.api.nvim_create_namespace('ffs_preview_location')
  end
end
...
local function ensure_hex_ns()
  if not hex_ns then hex_ns = vim.api.nvim_create_namespace('ffs_hex_preview') end
  return hex_ns
end
