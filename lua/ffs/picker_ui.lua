...
local conf = require('ffs.conf')
local file_picker = require('ffs.file_picker')
local preview = require('ffs.file_picker.preview')
local utils = require('ffs.utils')
local location_utils = require('ffs.location_utils')
local combo_renderer = require('ffs.combo_renderer')
local list_renderer = require('ffs.list_renderer')
local scrollbar = require('ffs.scrollbar')
local rust = require('ffs.rust')
...
--- @return string|nil
local function canize_ffs_path(relative_path)
...
--- @return string|nil
local function resolve_item_path(item) return item and canize_ffs_path(item.relative_path) or nil end
...
function M.create_ui()
  ...
  if not M.state.ns_id then
    M.state.ns_id = vim.api.nvim_create_namespace('ffs_picker_status')
...
function M.setup_buffers()
  vim.api.nvim_buf_set_name(M.state.input_buf, 'ffs search')
  vim.api.nvim_buf_set_name(M.state.list_buf, 'ffs list')
  if M.enabled_preview() then vim.api.nvim_buf_set_name(M.state.preview_buf, 'ffs preview') end

  vim.api.nvim_set_option_value('buftype', 'prompt', { buf = M.state.input_buf })
  vim.api.nvim_set_option_value('filetype', 'ffs_input', { buf = M.state.input_buf })
  ...
  vim.api.nvim_set_option_value('filetype', 'ffs_list', { buf = M.state.list_buf })
  ...
  if M.state.file_info_buf then
    ...
    vim.api.nvim_set_option_value('filetype', 'ffs_file_info', { buf = M.state.file_info_buf })
  ...
  if M.enabled_preview() then
    ...
    vim.api.nvim_set_option_value('filetype', 'ffs_preview', { buf = M.state.preview_buf })
...
function M.setup_windows()
  ...
  local picker_group = vim.api.nvim_create_augroup('ffs_picker_focus', { clear = true })
...
function M.get_suggestion_renderer()
  if M.state.suggestion_source == 'grep' then
    return require('ffs.grep.grep_renderer')
  else
    return require('ffs.file_renderer')
  end
end
  ...
  if M.state.mode == 'grep' then
    ...
    if next(M.state.selected_files) then
      for relative_path, _ in pairs(M.state.selected_files) do
        table.insert(paths, canize_ffs_path(relative_path))
      end
    else
    ...
    if query and query ~= '' then
      ...
      if config.history and config.history.enabled then
        ...
        if mode == 'grep' then
          ...
          pcall(ffs.track_query_completion, query, item.relative_path)
        end
      end
    end
  end)
...
function M.close()
  ...
  local ts_ok, ts_hl = pcall(require, 'ffs.treesitter_hl')
  ...
  pcall(vim.api.nvim_del_augroup_by_name, 'ffs_picker_focus')
end
