...
function M.find_files(opts)
  local picker_ok, picker_ui = pcall(require, 'ffs.picker_ui')
  if picker_ok then
    picker_ui.open(opts)
  else
...
function M.live_grep(opts)
  ...
  local grep_renderer = require('ffs.grep.grep_renderer')
...
function M.open_file_under_cursor(open_cb)
  ...
  picker_ui.open_with_callback(full_path_with_suffix, function(files, _, location)
    if #files == 1 or require('ffs.file_picker').get_file_score(1).exact_match then
      ...
      if location then vim.schedule(function() require('ffs.location_utils').jump_to_location(location) end) end
