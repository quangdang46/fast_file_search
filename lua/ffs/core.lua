...
if not fuzzy then error('Failed to load ffs.fuzzy module. Ensure the Rust backend is compiled and available.') end

local M = {}

---@class ffs.core.State
local state = {
  ---@type boolean
  initialized = false,
  ---@type boolean
...
local function setup_global_autocmds(config)
  local group = vim.api.nvim_create_augroup('ffs_file_tracking', { clear = true })
...
M.ensure_initialized = function()
  ...
  local frecency_db_path = config.frecency.db_path or (vim.fn.stdpath('cache') .. '/ffs_frecency')
  local history_db_path = config.history.db_path or (vim.fn.stdpath('data') .. '/ffs_history')
  ...
  local git_utils = require('ffs.git_utils')
